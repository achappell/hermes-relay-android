...
""",
            ...
            self.assertEqual(
                bmad_issue_tracking.resolve_story_issue_id(
                    "1-a-1-start-an-authorized-hermes-turn",
                    host="github.com",
    ...
    def test_story_index_prefix_collision_fails_closed(self) -> None:
        with self.assertRaisesRegex(ValueError, "matches multiple"):
            bmad_issue_tracking.find_story_record(
                {"1-A-1": {}, "1-A-1-START": {}}, "1-a-1-start-turn"
            )
    ...
    def test_search_uses_exact_sprint_key_ignores_prs_and_prefix_titles(self) -> None:
        payload = {
            "items": [
                {
                    "number": 15,
                    "title": "1-a-1-start-turn",
                    "body": "**Sprint Key:** `1-a-1-start-turn`",
                ...
                {"number": 17, "title": "1-a-1-start-turn-duplicate", "body": ""},
            ]
        }

        self.assertEqual(
            bmad_issue_tracking.resolve_story_issue_id_from_search(
                json.dumps(payload), "1-a-1-start-turn"
            ),
            "16",
        )
    ...
    def test_search_matches_full_story_key_in_title(self) -> None:
        payload = {
            "items": [
                {"number": 18, "title": "Story 1-a-1-start-turn", "body": ""},
                {"number": 19, "title": "Story 1-a-1-start-turn-copy", "body": ""},
            ]
        }
        self.assertEqual(
            bmad_issue_tracking.resolve_story_issue_id_from_search(
    ...
    def test_search_ambiguous_match_fails_closed(self) -> None:
        payload = {
            "items": [
                {"number": 15, "body": "**Sprint Key:** `1-a-1-start-turn`"},
                {"number": 16, "body": "**Sprint Key:** `1-a-1-start-turn`"},
            ]
        }
    ...
    def test_search_rejects_unexpected_response_shape(self) -> None:
        with self.assertRaisesRegex(TypeError, "unexpected response"):
            bmad_issue_tracking.resolve_story_issue_id_from_search("[]", "1-a-1-x")
...
""",
            ...
            self.assertTrue(
                bmad_issue_tracking.record_story_issue_url(
                    "1-a-2-render-honest-turn-phases",
                    "27",
                    host="github.com",
                    project="example/hermes-client",
                    platform="github",
